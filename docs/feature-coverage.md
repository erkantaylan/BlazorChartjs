...
| autocolors | **Partial (raw JSON)** | The plugin is vendored and can be attached with `Options.RegisterPlugins = ["chartjs-plugin-autocolors"]` and configured through `Plugins.ExtraOptions["autocolors"]` ([Any other Chart.js plugin](plugins.md#any-other-chartjs-plugin)). There is no typed model: `Plugins` has no `autocolors` property, and the `Autocolors` class under `Models/Common` is still not referenced by anything. Its script is not in the README's install list, so add the tag yourself. |
