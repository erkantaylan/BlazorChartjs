...
| autocolors | **None** | The bundle is still vendored under `lib/`, but nothing registers it, its script is not in the install list, and there is no model for it — the unreferenced `Autocolors` class is gone. Chart.js 4's built-in `colors` plugin does the same job and is fully reachable as `Plugins.Colors`. |
