...
namespace Erkan.Blazor.Chartjs.Models.Common
{
    ...
    /// <see cref="InflateAmountJsonConverter"/> writes it that way. Assign a number directly:
    /// <c>InflateAmount = 0</c>. <c>default(InflateAmount)</c> is <see cref="Auto"/>, which is also
    ...
    public readonly struct InflateAmount
    {
        ...
        /// <summary>
        /// Inflates by 0.33 pixels when the bars have a fixed <c>barThickness</c>, or when
        /// <c>barPercentage</c> and <c>categoryPercentage</c> are both <c>1</c> so that bars touch —
        /// which hides the hairline anti-aliasing leaves between them — and by nothing otherwise.
        /// </summary>
